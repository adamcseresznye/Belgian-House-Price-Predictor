...
def get_last_page_number_from_url(url: str, session: HTMLSession) -> int:
    """
...
def get_links_to_listings(url: str, session: HTMLSession) -> Element:
    """
...
def extract_ads_from_given_page(
    links: Set[str], session: HTMLSession
) -> List[pd.DataFrame]:
    """
    ...
    for item in list(links.absolute_links):
        try:
            ...
            individual_ad = pd.concat(pd.read_html(r.text)).dropna().set_index(0)
...
def join_and_transpose_dataframes(dfs: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    # return pd.concat([ad for ad in dfs], axis=1).transpose()
...
def immoweb_scraping_pipeline(
    ...
    for page in tqdm(
        ...
        links = get_links_to_listings(url, session)
        # print(f"Getting links from page {page}.")

        # Parse data from the retrieved links
        parsed_data = extract_ads_from_given_page(links, session)

        # Join the parsed data into a single table
        joined_tables = join_and_transpose_dataframes(parsed_data)

        reformatted_dataframe = reformat_scraped_dataframe(
...
# last_page = get_last_page_number_from_url('https://www.immoweb.be/en/search/house/for-sale?countries=BE&page=1&orderBy=relevance', session)
